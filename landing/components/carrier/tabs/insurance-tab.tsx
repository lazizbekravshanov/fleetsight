          ...
          {[...groups.entries()].map(([type, policies]) => (
            <div key={type} className="rounded-lg border border-gray-200 bg-gray-50 p-3">
              ...
              {policies.map((p, i) => {
                ...
                return (
                  <div key={i} className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-gray-500 mb-1">
                    {coverage > 0 && (
                      <span>
                        Coverage:{" "}
                    ...
                    {p.name_company && (
                      <span>
                        Company: <span className="text-gray-700">{p.name_company}</span>
                      </span>
                    )}
                    {p.policy_no && (
                      <span>
                        Policy: <span className="text-gray-700">{p.policy_no}</span>
                      </span>
                    )}
                    {p.effective_date && (
                      <span>
                        Effective: <span className="text-gray-700">{p.effective_date}</span>
                      </span>
                    )}
                    {p.trans_date && (
                      <span>
                        Transaction: <span className="text-gray-700">{p.trans_date}</span>
                      </span>
                    )}
                    {p.docket_number && (
                      <span>
                        Docket: <span className="text-gray-700">{p.docket_number}</span>
                      </span>
                    )}
                  </div>
                );
            ...
            return (
              <div key={i} className="relative">
                ...
                <div className="text-xs">
                  ...
                  <div className="flex flex-wrap gap-x-4 text-gray-500 mt-0.5">
