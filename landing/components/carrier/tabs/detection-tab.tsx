...
function SignalSummaryBanner({ data }: { data: DetectionData }) {
  ...
  return (
    <div className="bg-[var(--surface-1)] border border-[var(--border)] shadow-sm rounded-2xl px-5 py-4 flex items-center justify-between gap-4">
      <div className="flex items-baseline gap-3">
        <span className="text-3xl font-extrabold tabular-nums text-[var(--ink)]">{totalSignals}</span>
        ...
        {SEVERITY_ORDER.map((sev) => {
          ...
          return (
            <div key={sev} className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] uppercase tracking-wide text-[var(--ink-muted)]">{sev}</span>
            </div>
          );
        })}
      </div>
...
function AnomalyFlagsCard({ flags }: { flags: AnomalyFlag[] }) {
  ...
  return (
    <div className={`bg-[var(--surface-1)] border border-[var(--border)] shadow-sm rounded-2xl p-5 ${glowClass}`}>
      <SectionHeader
        icon={<WarningIcon />}
        title="Anomaly Flags"
        count={flags.length}
            ...
            >
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-[var(--ink)]">{flag.label}</span>
                </div>
                <p className="mt-0.5 text-xs leading-relaxed text-[var(--ink-soft)]">{flag.detail}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={item.label} className="rounded-xl bg-orange-50 py-3 text-center">
            ...
            <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--ink-muted)]">
              {item.label}
            </p>
          </div>
        ))}
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        {reincarnation.priorDot && (
          <div className="flex items-center gap-2 rounded-xl bg-rose-50 px-4 py-2.5 ring-1 ring-rose-200">
            ...
            <span className="text-xs font-medium text-[var(--ink-soft)]">Prior DOT</span>
        ...
        <div className="flex items-center gap-3">
          {matchIndicators.map((m) => (
            <div key={m.key} className="flex flex-col items-center gap-1.5">
              ...
              <span className={`text-[10px] font-medium ${m.matched ? "text-rose-600" : "text-[var(--ink-muted)]"}`}>
                {m.label}
              </span>
            </div>
          ))}
    ...
    <div className="bg-[var(--surface-1)] border border-[var(--border)] shadow-sm rounded-2xl p-5">
      ...
      <div className="space-y-2">
        {matches.map((m) => (
          <div
            key={m.dotNumber}
            className="flex items-center justify-between rounded-lg border border-[var(--border)] bg-[var(--surface-2)] px-4 py-2.5"
          >
            <div className="min-w-0">
        ...
        {policies.map((si) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-[var(--ink-muted)]">{si.insurerName}</span>
            </div>
